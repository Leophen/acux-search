...
```js
import {useList, HooksList} from 'baidu-acu-react-common';
```
...
如果配置了 tableConfig，则会使用 [antd table](https://ant.design/components/table-cn/) 展示表格，所以配置项可以使用 Acud table 的配置项，组件会进行透传，下面就几个常用的配置项做说明。

| 配置项            | 说明                 | 类型                                                                | 默认值 |
| ----------------- | -------------------- | ------------------------------------------------------------------- | ------ |
| *columns          | 表格列的配置描述     | [antdTable columns](https://ant.design/components/table-cn/#Column) | `--`   |
...
<https://react-app-common.now.baidu-int.com/#/hooks/UseList>
